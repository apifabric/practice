...
resourcename="practice"  # lower case, only
resourcegroup="practice_rg"
...
# git clone https://github.com/apilogicserver/practice.git
# cd practice
...
az group create --name practice_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group practice_rg --sku S1 --is-linux
az container create --resource-group practice_rg --name practice --image apilogicserver/practice:latest --dns-name-label practice --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//practice.westus.azurecontainer.io:5656'
